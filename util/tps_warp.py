...
def WarpImageByLandmark(source_hp, target_hp, img):
    xs, ys, _ = GetLandmarkCoord(source_hp)
    source_points = np.array([[xs[i], ys[i]] for i in range(len(xs))])
    xs, ys, _ = GetLandmarkCoord(target_hp)
    target_points = np.array([[xs[i], ys[i]] for i in range(len(xs))])
    new_img, _, _ = WarpImage_TPS(source_points, target_points, img)
    return new_img
